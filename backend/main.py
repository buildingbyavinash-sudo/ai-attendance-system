...
# FIXED PATH LOGIC FOR CLOUD DEPLOYMENT
...
async def register_user(
    ...
    with open(save_path, "wb") as f: f.write(await image.read())
    
    # Store relative path for database portability
    db_image_path = os.path.join("faces", filename)
...
async def delete_user(user_id: str):
    try:
        with get_db() as conn:
            ...
            row = cursor.fetchone()
            if row:
                img_path = os.path.join(BASE_DIR, row[0])
...
async def update_user(
    ...
    with get_db() as conn:
        ...
        if image:
            ...
            save_path = os.path.join(FACES_DIR, filename)
            with open(save_path, "wb") as f: f.write(await image.read())
            db_path = os.path.join("faces", filename)
